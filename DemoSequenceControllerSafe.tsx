...
export default function DemoSequenceControllerSafe({ 
  sequence, 
  autoPlay = false, 
  language = "en", 
  delay = 5000 
}: DemoSequenceControllerProps) {
  ...
  const currentTrace = useMemo(() => {
    return validSequence[currentIndex] || null;
  }, [validSequence, currentIndex]);

  // Convert overwatch-schema/ paths to schema trace format
  const getSchemaTrace = useCallback((trace: string) => {
  ...
  const isSchemaStep = useMemo(() => {
    return currentTrace?.startsWith('overwatch-schema/') || false;
  }, [currentTrace]);
  ...
  }, [currentTrace, isSchemaStep, getSchemaTrace]);

  // Auto-play functionality
  useEffect(() => {
    if (!autoPlay || currentIndex >= validSequence.length - 1 || validSequence.length === 0) {
      return;
    }
    
    // Reduce timeout to prevent long waits
    const safeDelay = Math.min(Math.max(delay, 1000), 10000); // Between 1-10 seconds
    const timer = setTimeout(() => {
      setCurrentIndex((prev) => Math.min(prev + 1, validSequence.length - 1));
    }, safeDelay);
    
    return () => clearTimeout(timer);
  }, [currentIndex, autoPlay, delay, validSequence.length]);
      ...
      <div className="bg-secondary/20 p-6 rounded-lg shadow-lg">
        <div className="bg-destructive/20 border border-destructive/30 text-destructive p-4 rounded-lg">
          <div className="font-medium">
            {language === 'en' ? 'No Sequence Data' : 'Sin Datos de Secuencia'}
          </div>
          <div className="text-sm mt-1">
            {language === 'en' 
              ? 'No demo sequence provided or sequence is empty'
              : 'No se proporcionó secuencia de demo o la secuencia está vacía'
            }
          </div>
        </div>
      </div>
    ...
    <div className="bg-secondary/20 p-6 rounded-lg shadow-lg space-y-6">
      ...
      <div className="flex justify-between items-center mt-4">
        <button
          ...
          className="px-4 py-2 bg-card border border-border rounded hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          ◀ {language === 'en' ? 'Prev' : 'Ant'}
        </button>
        <span className="text-sm text-muted-foreground">
          {language === 'en' ? 'Step' : 'Paso'} {currentIndex + 1} {language === 'en' ? 'of' : 'de'} {validSequence.length}
        </span>
        <button
          disabled={currentIndex === validSequence.length - 1}
          onClick={goToNext}
          className="px-4 py-2 bg-card border border-border rounded hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
